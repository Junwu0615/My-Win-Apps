...
[1] Account A： Administrator ( 系統管理員 )
  用途： 僅用於安裝驅動程式、系統更新、執行 setup.ps1 此類會修改系統核心的腳本
  習慣： 平時不用這個帳號上網，也不把它當作日常帳號

[2] Account B： UserName ( 標準使用者 )
  用途： 日常開發、GitHub 操作、瀏覽網頁
  防禦力：因為不是管理員，病毒若想寫入 C:\Windows 或修改系統註冊表，
...
| 2. 備份鏡像 | Macrium Reflect (.mrimg) | 在乾淨環境下製作一次快照 ( USB )，作為備援防線 |
| * 恢復流程 | 直接還原鏡像 | 鏡像恢復 ( 該快照包含穩定的系統底層 + setup.ps1 ) |
